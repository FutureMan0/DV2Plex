...
{
	if (char *payload = _strdup("mailto:petr@mourek.cz?subject=WinDV"))
	{
		_beginthread(UrlThread, 0, payload);
	}
}

void CAboutDlg::OnUrl() 
{
	if (char *payload = _strdup("http://windv.mourek.cz/"))
	{
		_beginthread(UrlThread, 0, payload);
	}
}
...
LRESULT CDVToolsDlg::OnDVTimeChange(WPARAM, LPARAM lParam)
{
	char buf[100] = "";
	if (lParam > 0) {
		time_t stamp = static_cast<time_t>(lParam);
		const tm* timeInfo = localtime(&stamp);
		if (timeInfo)
		{
