  ...
  m_reader(NULL), m_thread(NULL)
{
	...
	while (!flnms.IsEmpty()) {
		...
		flnms = (filename.GetLength() < flnms.GetLength()) ? flnms.Mid(filename.GetLength() + 1) : CString();
...
CString FormatTime(LPCSTR format, time_t tim)
{
    ...
    // format ist bereits ein LPCSTR
    strftime(buf, sizeof buf, format, localtime(&tim));
